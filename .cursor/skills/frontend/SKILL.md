...
description: >-
    ...
    pages, templates, islands, components ou styles dans apps/dashboard.
---

# Front

Appliquer ces regles a chaque tache front du projet. Lire aussi [css-order.md](css-order.md) avant d'ecrire du CSS.
...
Stack: Deno, `@webtools/slick-server`, Preact, CSS dans `apps/dashboard/src/static/styles/`.
...
Le slot de page est `div#app` (pas un `main`) : Slick y injecte le body de la page. Reserver `main`, `header`, `footer`,
`nav` aux pages ou a leur contenu injecte.

Styler la coque via `#root` dans le CSS du template concerne (`auth.css`, `app.css`).

### Semantique et wrappers

1. `main`, `header`, `footer`, `nav` : une seule fois dans la page injectee (ou son contenu), pas en double. Le template
   expose seulement `div#app`.
2. Les pages injectent leur contenu dans `div#app` avec `section`, `main`, `article`, puis elements semantiques ou `div`
   / `span` si besoin.
...
Exemple template auth (slot Slick, partagé login / register / forgot-password) :

```tsx
<div id="app">{/* contenu page */}</div>;
```

Exemple page register (layout propre à l'inscription) :
...
- Un fichier CSS par page ou zone (`auth/register.css`, `auth.css`, `app.css`) declare dans `styles` du template/page.
...
- Preferer un component/island clair plutot que copier-coller.
...
- [ ] `main` / `header` / `footer` / `nav` une seule fois dans le contenu page, sans doublon
